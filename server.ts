...
function loadDb() {
  try {
    if (fs.existsSync(DB_FILE)) {
      ...
      db = {
        students: parsed.students || defaultRecords.students,
        faculty: parsed.faculty || defaultRecords.faculty,
        admins: parsed.admins || defaultRecords.admins,
        notices: parsed.notices || defaultRecords.notices,
        files: parsed.files || defaultRecords.files,
        scrapedUrls: parsed.scrapedUrls || defaultRecords.scrapedUrls
      };
    } else {
      saveDb();
    }
  } catch (err) {
    console.error("Error loading mock database, playing safe:", err);
  }
}
...
function saveDb() {
  try {
    ...
    console.error("Error storing mock database:", err);
  }
}
...
// Clean out legacy mock seed students who do not have a secure Access PIN
db.students = db.students.filter(s => s.pin !== undefined);
saveDb();
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (role === "student") {
    const student = db.students.find(s => s.regNo === regNo || s.email.toLowerCase() === (email || "").toLowerCase());
    if (student) {
    ...
    const fac = db.faculty ? db.faculty.find(f => f.email.toLowerCase() === (email || "").toLowerCase().trim()) : null;
    if (fac) {
      if (password === fac.pin) {
    ...
    const adm = db.admins ? db.admins.find(a => a.email.toLowerCase() === (email || "").toLowerCase().trim()) : null;
    if (adm) {
      if (password === adm.pin) {
...
app.get("/api/students/extract-knowledge", async (req, res) => {
  ...
  // 1. See if the student already exists in db.students
  let student = db.students.find(s => s.regNo === regNo);
  ...
  if (student && (!student.subjects || student.subjects.length === 0)) {
    ...
    const totalWPoints = student.subjects.reduce((sum, s) => sum + (s.credits * s.gradePoint), 0);
    ...
    const sumPastSgpa = student.sgpaHistory.reduce((sum, s) => sum + s.sgpa, 0);
  ...
  if (ai) {
    try {
      const relevantDocs = db.files.filter(f => f.extractedText && (f.extractedText.includes(regNo) || f.fileName.includes("Syllabus") || f.fileName.includes("Grades")));
...
app.post("/api/students", (req, res) => {
  ...
  const idx = db.students.findIndex(s => s.regNo === record.regNo);
  if (idx !== -1) {
    db.students[idx] = record;
  } else {
...
app.post("/api/faculty", (req, res) => {
  ...
  const idx = db.faculty.findIndex(f => f.email.toLowerCase() === record.email.toLowerCase());
  if (idx !== -1) {
    db.faculty[idx] = record;
  } else {
    db.faculty.push(record);
  }
  saveDb();
  res.json({ success: true, profile: record });
});
...
app.post("/api/admins", (req, res) => {
  ...
  const idx = db.admins.findIndex(a => a.email.toLowerCase() === record.email.toLowerCase());
  if (idx !== -1) {
    db.admins[idx] = record;
  } else {
    db.admins.push(record);
  }
  saveDb();
  res.json({ success: true, profile: record });
});
...
app.delete("/api/students/:regNo", (req, res) => {
  ...
  db.students = db.students.filter(s => s.regNo !== regNo);
  saveDb();
...
app.post("/api/notices", (req, res) => {
  ...
  db.notices.unshift(freshNotice);
  saveDb();
  res.json({ success: true, notice: freshNotice });
});
...
app.post("/api/files/upload", async (req, res) => {
        ...
        if (rows && rows.length > 0) {
          ...
          for (let r = startRow; r < rows.length; r++) {
            ...
            if (foundRegNo) {
              ...
              const existingStudentIdx = db.students.findIndex(s => s.regNo === foundRegNo);
              if (existingStudentIdx !== -1) {
                const cur = db.students[existingStudentIdx];
                cur.name = foundName || cur.name;
                cur.cgpa = computedSgpa;
                if (foundSubjectsList.length > 0) {
                  cur.subjects = foundSubjectsList;
                }
                cur.attendanceRate = cur.attendanceRate || 85; 
              } else {
                db.students.push({
                  regNo: foundRegNo,
  ...
  freshFile.extractedText = extractedText;
  freshFile.summary = summary;

  db.files.push(freshFile);
  saveDb();
  res.json({ success: true, file: freshFile });
});
...
app.post("/api/scraper/crawl", async (req, res) => {
  ...
  db.scrapedUrls = db.scrapedUrls.filter(p => p.url !== url);
  db.scrapedUrls.push(freshPage);
  saveDb();
  res.json({ success: true, page: freshPage });
});

app.delete("/api/scraper", (req, res) => {
  const { url } = req.body;
  db.scrapedUrls = db.scrapedUrls.filter(s => s.url !== url);
  saveDb();
...
app.post("/api/chat/query", async (req, res) => {
  ...
  let studentProfile: StudentRecord | undefined;
  if (role === "student" && regNo) {
    studentProfile = db.students.find(s => s.regNo === regNo);
    if (studentProfile) {
...
${studentProfile.subjects.map(s => `- ${s.code}: ${s.name} (Credits: ${s.credits}, Grade: ${s.grade}, Grade Point: ${s.gradePoint})`).join("\n")}
`;
    }
  }
  ...
  if (ai) {
    ...
    if (query.toLowerCase().includes("cgpa") || query.toLowerCase().includes("sgpa") || query.toLowerCase().includes("grade") || query.toLowerCase().includes("credit")) {
      if (studentProfile) {
        ...
        responseText += `Our calculation models show that you have successfully achieved a cumulative total of **${studentProfile.subjects.reduce((sum, s) => sum + s.credits, 0)}** registered credits in your active semester.`;
      } else {
